...
Current content includes files 01 through 39 and 41 through 109. File 40 was not supplied. Collection-level figures originate in supplied editorial copy and have not been independently reconciled against an inventory spreadsheet. Related holdings may overlap. The public register records `verifiedImageCount: null` until source reconciliation is complete. Broad subject collections are not marked as individual historical events. ZIP-batch README notes remain production context and are not published as page copy.

The importer supports YAML front matter and labelled Markdown metadata, including both frame-count labels used in the source files. Suggested `/archive/` routes in Batches 10 through 12 are mapped into the existing `/photography-archive/` structure, including links to earlier collections. Supplied image-treatment instructions, sample JSON-LD and physical storage locations stay in private source files; the build generates schema consistently from public page data. The garden and botanical parent collection links to its ten detailed collections; hair and grooming links to its twelve detailed collections. Interiors and design links to ten detailed household collections. Each child links back to the appropriate parent without duplicate related links. Source numbers may contain three or more digits.
