...
for(const p of pages){
 ...
 if(p.path!=='/')graph.push({'@type':'BreadcrumbList','@id':base+p.path+'#breadcrumbs',itemListElement:breadcrumbs.map((b,i)=>({'@type':'ListItem',position:i+1,name:b.label,item:base+b.path}))});
 ...
 const hero= `<header class="page-heading ${p.hero?'with-image':'type-only'}"><div><p class="eyebrow">${p.id==='home'?'The Robert Harper Photograph Library':esc(p.title)}</p><h1>${esc(p.h1)}</h1>${p.id==='home'?'<p class="hero-intro">Archive.London, home of the Robert Harper Photograph Library.</p><a class="button" href="/photography-archive/">Explore the archive <span aria-hidden="true">↗</span></a>':`<p class="page-deck">${esc(p.metaDescription)}</p>`}</div>${p.hero?figure(p.hero,true):`<div class="type-mark" aria-hidden="true">${p.pageType==='biography'?'RH':p.id==='photography-archive'?'A-Z':esc(p.title.slice(0,1))}</div>`}</header>`;
 const toc=rendered.headings.filter(h=>h.level==='h2');
 const relatedHTML=publishedRelated.length?`<section class="related"><p class="eyebrow">Continue exploring</p><h2>Related collections</h2><ul>${publishedRelated.map(r=>`<li><a href="${r.url}">${esc(r.label)} <span aria-hidden="true">↗</span></a></li>`).join('')}</ul></section>`:'';
